...
역할 기반 접근 제어: 구매자/판매자 분리된 기능 및 화면
배터리 정보 관리: 제조사, 모델, 제품 번호 등록 및 관리
QR 코드 스캔: 카메라를 활용한 배터리 QR 코드 등록
BLE 장치 연결: 블루투스 저전력 장치 검색 및 페어링
데이터 수집 및 전송: 센서 데이터 실시간 모니터링 및 서버 전송
진단 기능: 수집된 데이터 기반 배터리 상태 진단
사용자 설정: 프로필 관리, 비밀번호 변경, 로그아웃
...
안드로이드 버전별 BLE 권한 처리: Android 6.0부터 13.0까지 버전별 다른 권한 요청 로직
JWT 토큰 관리: 토큰 자동 갱신 및 보안 저장
바이너리 프로토콜: 센서 데이터 수집을 위한 커스텀 프로토콜 구현
포그라운드 서비스: 앱 백그라운드 상태에서도 BLE 연결 유지
Daum 주소 API: 사업자 및 개인 주소 검색 기능
MVVM 아키텍처: ViewModel, Repository, Service 계층 분리
반응형 UI: Jetpack Compose와 Flow를 활용한 상태 관리
다중 사용자 타입: 구매자/판매자별 다른 워크플로우 지원
타임아웃 처리: 블루투스 통신 안정성을 위한 타임아웃 메커니즘
배터리 데이터 가공: 센서 데이터 정규화 및 서버 전송 포맷 변환
